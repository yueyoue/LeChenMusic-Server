...
router.get('/', authMiddleware, async (req, res, next) => {
  try {
    const type = qs(req.query.type) as 'track' | 'album' | 'artist' | undefined;
  ...
  try {
    await db.delete(schema.favorite).where(
      and(
        ...
        eq(schema.favorite.targetType, req.params.type as string as 'track' | 'album' | 'artist'),
        eq(schema.favorite.targetId, parseInt(req.params.targetId as string)),
      )
    );
    res.json({ code: 0, message: 'ok', data: null });
  } catch (err) { next(err); }
